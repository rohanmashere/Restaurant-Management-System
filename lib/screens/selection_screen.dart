...
import 'package:delight_corner/screens/waiter_home_screen.dart';
import 'package:delight_corner/screens/waiter_login_screen.dart';
  ...
  void waiter() async {
    try {
      ...
      for (var userDoc in userDocs.docs) {
        ...
        if (userData != null && userData.containsKey('waiters')) {
          ...
          for (var waiter in waiters) {
            if (waiter['active'] == true &&
                deviceCode == waiter['deviceCode']) {
      ...
      if (loggedInWaiter != null) {
        ...
        Navigator.of(context).push(
          MaterialPageRoute(
            builder: (ctx) => WaiterLoginScreen(),
          ),
        );
      }
    } catch (e) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Failed to check waiter login status.')),
      );
    }
  }
